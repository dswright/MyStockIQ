...
class HelperWorker
  ...
  #     offset = tz.parse(new_time_string.utc_time_full).utc_offset() #get the offset amount from EST. Could be 4 or 5 hours depending on DSt
  ...
  def perform(ticker_symbol) #update the graph_time in the daily table. Delete intraday table to take care of those.
    ...
    stockprices.each do |stockprice|
      ...
      old_date = stockprice.date
      graph_time = old_date.utc_time_int * 1000
