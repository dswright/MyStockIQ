...
#this function recieves, the current stock array, and updates it with the stock industries too.
  def perform(stock_array)
    ...
    stock_array.each do |stock|
      ...
      csv_stock_row = stock_industry_array.detect {|a| a[:ticker_symbol]==stock["ticker_symbol"]}
      ...
      unless csv_stock_row.nil?
        ...
        unless stock_to_update.nil?
          stock_to_update.stock_industry = csv_stock_row[:stock_industry]
          stock_to_update.exchange = csv_stock_row[:exchange]
          stock_to_update.save
        end
      end
    end
    stock_industry_array = nil
    stock_array = nil
    stock_to_update = nil
  end
end